...
import type { PublicProjectItem } from '../composables/usePublicContent';
      ...
      <div class="container projects-page__content">
        <template v-if="projectItems.length">
          ...
          <div v-if="viewMode === 'grid' && filteredProjects.length" class="grid grid--3">
            <article v-for="item in filteredProjects" :key="item.id" class="post-card post-card--project" style="cursor: default">
                ...
                <tbody>
                  ...
                  >
                    <td style="min-width: 200px">
                      <div style="font-weight: 700; display: flex; gap: 0.35rem; flex-wrap: wrap; align-items: center">
                        {{ item.title }}
                      </div>
                      <div style="font-size: var(--text-xs); color: var(--color-text-muted); margin-top: 0.25rem; line-height: 1.45">
                        {{ item.summary }}
                      </div>
                    </td>
                    <td>
                      <div style="display: flex; gap: 0.35rem; flex-wrap: wrap">
                        ...
                        <span v-if="!item.stack.length" style="font-size: var(--text-xs); color: var(--color-text-muted)">—</span>
                      </div>
                    </td>
                    <td>
                      <span class="badge" :class="statusClass(item.status)">{{ statusLabel(item.status) }}</span>
                    </td>
                    <td style="color: var(--color-text-muted); font-size: var(--text-sm); white-space: nowrap">
