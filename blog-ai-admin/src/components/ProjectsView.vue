...
<template>
  <section style="display:flex;flex-direction:column;gap:1.5rem;">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
      <div>
        <h2 style="font-size:var(--text-2xl);font-weight:800;">项目分享</h2>
        <p style="color:var(--color-text-muted);margin-top:.35rem;">与前台「项目分享」一致；列表查看，需要时在弹框中编辑。</p>
      </div>
      <button class="btn btn--primary" type="button" @click="$emit('openProjectModal')">添加项目</button>
    </div>
          ...
          <tbody>
            <tr v-for="item in projects" :key="item.id">
              <td style="min-width:240px;">
                <div style="font-weight:700;display:flex;gap:.4rem;align-items:center;flex-wrap:wrap;"><span>{{ item.title }}</span></div>
                <div style="font-size:var(--text-xs);color:var(--color-text-muted);margin-top:.25rem;">{{ item.summary }}</div>
              </td>
              ...
              <td>
                <div style="display:flex;gap:.4rem;flex-wrap:wrap;">
                  ...
                  <span v-if="!item.stack.length" style="font-size:var(--text-xs);color:var(--color-text-muted);">未设置</span>
                </div>
              </td>
              <td style="color:var(--color-text-muted);font-size:var(--text-sm);">{{ formatDate(item.updatedAt) }}</td>
              <td>
                <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
                  <button class="btn btn--ghost btn--sm" type="button" @click="openEditor(item)">编辑</button>
                  <button class="btn btn--ghost btn--sm" type="button" style="color:var(--color-danger);" @click="$emit('remove', item.id)">删除</button>
                </div>
              </td>
            </tr>
            <tr v-if="!projects.length">
              <td colspan="5" style="text-align:center;color:var(--color-text-muted);padding:2rem 1rem;">当前还没有项目内容</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="modal-overlay" :class="{ open: editorOpen }">
      <div class="modal" style="max-width:880px;">
        ...
        <div v-if="editor.id" class="modal-split">
          <form class="admin-form" @submit.prevent="handleSave">
            ...
            <div class="form-group">
              ...
              <textarea v-model="editor.summary" class="form-textarea" style="min-height:120px;"></textarea>
            </div>
            <div class="form-group">
          ...
          <div class="panel" style="overflow:hidden;">
            ...
            <div class="panel-body" style="display:flex;flex-direction:column;gap:1rem;">
              ...
              <div v-if="!editor.sourceUrl && !editor.githubUrl" style="color:var(--color-text-muted);font-size:var(--text-sm);">项目网址和 GitHub 地址会显示在这里。</div>
            </div>
          </div>
        </div>
      </div>
